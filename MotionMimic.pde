// Import OpenCV and video libraries for image processing and camera capture
import gab.opencv.*;
...
void draw() {
  ...
  opencv.threshold(100); // Apply a threshold to highlight shapes// Originial Code
  thresholdImg = opencv.getOutput(); // Get the processed image
  image(thresholdImg, 0, 0); // Display the thresholded image on screen
 
  // Draw a rectangle on the right side to contain generated artwork circles
  stroke(0);
  noFill();
  rect(640, 0, 160, 480);
  ...
  // Find contours in the thresholded image// Goncharov 1. (2022)
  ArrayList<Contour> contours = opencv.findContours(); // Bleed AI Academy. Contour Detection In OpenCV 101
  ...
  for (Contour contour : contours) {
    ...
    if (contourArea > 3000) { // Minimum area threshold to filter out small shapes
      ...
      if (aspectRatio > 0.95 && aspectRatio < 1.05 && circularity > minCircularity && width >= minCircleRadius * 2) {
        ...
        // Display a message indicating circle gesture detection// Goncharov I. (2022).
        fill(255, 0, 0);
        textSize(20);
        ...
        if (!circleDetectedLastFrame) { // Only add a circle if it wasn’t detected in the last frame
          ...
          // Add a new circle with random color to artwork
          artworkCircles.add(new Circle(randomX, randomY, 100, color(random(255), random(255), random(255))));
        }
         // "OpenCV Circles Detection with Threshold, Edges and Contours in Python" (gsnikitin, 2017)
      }
    }
  }

  // Update the last frame's circle detection status // OpenCV Circles Detection with Threshold, Edges and Contours in Python (gsnikitin, 2017)
  circleDetectedLastFrame = circleDetected;

  // Display each artwork circle in the list // OpenCV Circles Detection with Threshold, Edges and Contours in Python (gsnikitin, 2017)
...
class Circle {
  ...
  // Display method to draw the circle // OpenCV Circles Detection with Threshold, Edges and Contours in Python (gsnikitin, 2017)
  void display() {
